...
import { dateInZone, displayTime, navigateDate, shiftDate, validZone, viewWindow } from "../time";
...
export function Schedules({ id }: {
    ...
    const list = useQuery({ queryKey: [...keys.schedules(id), range.from, range.to, page], queryFn: () => api.schedules(id, range.from, range.to, page), enabled: !!project.data });
    ...
    const filtered = enriched.filter(r => (!text || r.s.title.toLocaleLowerCase().includes(text.toLocaleLowerCase()) || r.s.description?.toLocaleLowerCase().includes(text.toLocaleLowerCase())) && (status === "ALL" || r.s.status === status) && (ack === "ALL" || r.ack === ack) && (calendar === "ALL" || r.calendar === calendar) && (!mine || r.s.createdBy === me.data!.id) && (!after || dateInZone(r.s.endsAt, zone) >= after) && (!before || dateInZone(r.s.startsAt, zone) <= before));
    ...
    <div className="toolbar"><label>검색<input value={text} onChange={e => setText(e.target.value)}/></label><label>날짜 이후<input type="date" value={after} onChange={e => setAfter(e.target.value)}/></label><label>날짜 이전<input type="date" value={before} onChange={e => setBefore(e.target.value)}/></label>
    ...
    <QueryState query={list}/><QueryState query={connection} label="연결 다시 시도"/>
    ...
    {list.isSuccess && <><h2>{view === "month" ? "월간 신호 보기" : "주간 실제 일정"}</h2><p>{range.days[0]} ~ {shiftDate(range.days[range.days.length - 1], 0)} · {zone}</p>
    ...
    </div><h2>일정 목록</h2>{!filtered.length && <p>조건에 맞는 일정이 없습니다.</p>}<ul className="schedule-list">{filtered.map(r => <li key={r.s.id}><Link to={`/projects/${id}/schedules/${r.s.id}`}>{r.s.title}</Link><time dateTime={r.s.startsAt}>{displayTime(r.s.startsAt, zone)}</time><span>~ {displayTime(r.s.endsAt, zone)}</span><span>상태 {r.s.status}</span><span>확인 {r.ack}</span><span>Calendar {r.calendar ?? (r.projection?.isError || connection.isError ? "불러오기 실패" : "불러오는 중")}</span></li>)}</ul>
