...
public class DriveService {
 ...
 private final GoogleAccess access; private final GoogleHttpClient http; private final DriveReferenceRepository refs; private final ProjectAccess projects; private final com.aierp.identity.api.IdentityProfiles profiles;
 public DriveService(GoogleAccess access,GoogleHttpClient http,DriveReferenceRepository refs,ProjectAccess projects){this(access,http,refs,projects,null);}
 @Autowired public DriveService(GoogleAccess access,GoogleHttpClient http,DriveReferenceRepository refs,ProjectAccess projects,com.aierp.identity.api.IdentityProfiles profiles){this.access=access;this.http=http;this.refs=refs;this.projects=projects;this.profiles=profiles;}
 public FilesResponse files(UUID user,String query,String pageToken){
   ...
   var response=http.execute("GET",URI.create(uri),credential.accessToken(),null); check(response); JsonNode root=parse(response.body()); var list=new ArrayList<FileItem>();
   int count=0;
 ...
 public ProjectFilesResponse projectFiles(UUID project,UUID user,int page){
   ...
   return new ProjectFilesResponse(p.getContent().stream().map(r->toProject(r,user,role)).toList(),p.hasNext());
 }
 ...
 private ProjectFile toProject(DriveReferenceEntity r,UUID u,String role){String label="알 수 없는 사용자";if(profiles!=null){var p=profiles.find(List.of(r.attachedBy)).get(r.attachedBy);if(p!=null)label=p.displayName()==null||p.displayName().isBlank()?p.email():p.displayName();}return new ProjectFile(r.id,r.fileId,r.name,r.mimeType,r.url,label,r.attachedAt,r.attachedBy.equals(u)||"MANAGER".equals(role));}
 ...
 private static void check(GoogleHttpClient.Response r){if(r.status()==401)throw new GoogleAccess.GoogleAccessException(GoogleAccess.Status.REAUTH_REQUIRED);if(r.status()==403)throw new GoogleAccess.GoogleAccessException(GoogleAccess.Status.PERMISSION_REQUIRED);if(r.status()<200||r.status()>=300)throw new GoogleHttpClient.GoogleServiceException("PROVIDER_UNAVAILABLE");}
